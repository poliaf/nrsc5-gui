...
# noinspection PyAttributeOutsideInit,PyShadowingBuiltins,PyUnusedLocal
class NRSC5_GUI(object):
        ...
        if not self.playing:
            ...
            if self.stationLogos.has_key(self.stationStr):
                ...
                if os.path.isfile(logo):
                    ...
                    pixbuf = GdkPixbuf.Pixbuf.scale_simple(pixbuf, 200, 200, GdkPixbuf.InterpType.HYPER)
    ...
    def on_bookmarkNameEdited(self, cell, path, text, *data):
        ...
        if btn.get_active():
            if btn == self.radMapTraffic:
                ...
                if os.path.isfile(self.mapData["weatherNow"]):
                    ...
                    self.imgMap.set_from_pixbuf(imgToPixbuf(mapImg, True))
                else:
                    self.imgMap.set_from_stock(Gtk.STOCK_MISSING_IMAGE, Gtk.IconSize.LARGE_TOOLBAR)
        ...
        if m:
            ...
            try:
                ...
                if self.radMapWeather.get_active():
                    imgMap = imgMap.resize((200,200), Image.LANCZOS)                                    # scale map to fit window
                    self.imgMap.set_from_pixbuf(imgToPixbuf(imgMap, True))                                    # convert image to pixbuf and display
    ...
    def animate(self):
        ...
        if os.path.isfile(fileName):
            ...
            if self.config["scale"]:
                mapImg = imgToPixbuf(Image.open(fileName).resize((600,600), Image.LANCZOS), True)             # open weather map, resize to 600x600, and convert to pixbuf
            else:
                mapImg = imgToPixbuf(Image.open(fileName), True)                                              # open weather map and convert to pixbuf
...
def imgToPixbuf(img, has_alpha=False):
